...
#setting.py와 같은 웹서버 로드가 완전히되지않은 공간에서
#별칭 기반의 URL검색을 사용하려면 reverse_lazy 함수를 사용해야함
LOGIN_REDIRECT_URL = reverse_lazy('main')
#비로그인상태에서 로그인이 필요한 기능을 사용했을때 로그인페이지를
#띄우기 위해 로그인페이지의 URL을 등록하는 변수
LOGIN_URL = reverse_lazy("cl:signin")

#외부 웹클라이언트가 웹서버에게 요청할때 허용되는 도메인주소를 저장하는 변수
#웹서버에 접근할수 있는 도메인 주소는 127.0.0.1과 xxx.pythonanywhere.com으로만 요청할수있음
...
#구글로그인하기에 사용할 요청 ID
SOCIAL_AUTH_GOOGLE_OAUTH2_KEY = '580693676581-ko3l2sm5ukvlbnvs6a28soabvjdcv1d0.apps.googleusercontent.com'
#요청 PASSWORD
SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET = 'G6AOo3ZrpgJuGyqa-uXFgZQ1'
